...
- [`Puppeteer`](#class-puppeteer) 使用 [DevTools 协议](https://chromedevtools.github.io/devtools-protocol/) 与浏览器通信
- [`Browser`](#class-browser) 实例可以拥有多个浏览器 context
- [`BrowserContext`](#class-browsercontext) 实例定义了一个浏览会话，同时可拥有多个页面
- [`Page`](#class-page) 至少拥有一个框架（主框架），可以通过 [iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe) 或者 [frame](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/frame) 标签创建更多的框架
- [`Frame`](#class-frame) 至少拥有一个可执行上下文 （默认可执行上下文），Frame 的 Javascript 在此可执行上下文中运行。 一个 Frame 可能有额外的与[扩展](https://developer.chrome.com/extensions)关联的执行上下文
- [`Worker`](#class-worker) 具有单个执行上下文，可以很方便的与 [WebWorkers](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API) 交互

(图表资源: [link](https://docs.google.com/drawings/d/1Q_AM6KYs9kbyLZF-Lpp5mtpAWth73Cq8IKCsWYgi8MM/edit?usp=sharing))
...
Puppeteer 会使用一些 [环境变量](https://en.wikipedia.org/wiki/Environment_variable) 来帮助其操作。这些变量既可以在环境中设置，也可以在 [npm 配置](https://docs.npmjs.com/cli/config) 中设置。
...
- `PUPPETEER_SKIP_CHROMIUM_DOWNLOAD` - 安装过程中不要下载捆绑的 Chromium
- `PUPPETEER_DOWNLOAD_HOST` - 覆盖用于下载 Chromium 的 URL 的主机部分
- `PUPPETEER_CHROMIUM_REVISION` - 在安装步骤中指定 puppeteer 使用的 chrome 版本

### class: Puppeteer

<!-- Puppeteer module provides a method to launch a Chromium instance.
The following is a typical example of using Puppeteer to drive automation: -->
Puppeteer 模块提供了一个启动 Chromium 实例的方法。
下面是使用 Puppeteer 驱动自动化测试的典型示例
```js
...
- `options` <[Object]>
  - `browserWSEndpoint` <[string]> 指定连接的[浏览器 websocket endpoint](#browserwsendpoint)
  - `ignoreHTTPSErrors` <[boolean]> 是否忽略 HTTPS 错误，默认 `false`.
  - `slowMo` <[number]> 指定 Puppeteer 操作间隔的时间，使其变慢，单位为毫秒。需要观察操作过程的时候设置
- 返回值: <[Promise]<[Browser]>>

<!-- This methods attaches Puppeteer to an existing Chromium instance. -->
此方法将 Puppeteer 附加到一个已有的 Chromium 实例中。
...
- 返回值: <[Array]<[string]>> Chromium 启动的默认 flags
...
- `options` <[Object]>  设置 browser 的可配置选项。 可以有以下字段：
  - `ignoreHTTPSErrors` <[boolean]> 是否忽略 HTTPS 错误，默认 `false`
  - `headless` <[boolean]> 是否以 [headless模式] (https://developers.google.com/web/updates/2017/04/headless-chrome)运行浏览器，默认为 true，除非 devtools 选项为 true
  - `executablePath` <[string]> 使用指定路径的 Chromium 或者 Chrome 可执行文件代替捆绑的 Chromium，如果 `executablePath` 是一个相对路径，那么它是相对于[当前工作目录](https://nodejs.org/api/process.html#process_process_cwd)
  - `slowMo` <[number]> 指定 Puppeteer 操作间隔的时间，使其变慢，单位为毫秒。需要观察操作过程的时候设置
  - `args` <[Array]<[string]>> 传递给浏览器实例的其他参数，Chromium 的 flags 列表可以在[这里](http://peter.sh/experiments/chromium-command-line-switches/)找到
  - `ignoreDefaultArgs` <[boolean]> 不使用 [`puppeteer.defaultArgs()`](#puppeteerdefaultargs)，危险参数，谨慎使用，默认 `false`
  - `handleSIGINT` <[boolean]> Ctrl-C 关闭浏览器进程，默认 `true`
  - `handleSIGTERM` <[boolean]> 浏览器进程收到 SIGTERM 信号关闭，默认 `true`
  - `handleSIGHUP` <[boolean]> 浏览器进程收到 SIGHUP 信号关闭，默认 `true`
  - `timeout` <[number]> 浏览器实例启动超时时间，单位毫秒，默认 `30000` (30 秒)，设置为 `0` 一直等待启动为止
  - `dumpio` <[boolean]> 是否将浏览器进程标准输出和标准错误输入到 `process.stdout` 和 `process.stderr` 中， 默认 `false`
  - `userDataDir` <[string]> 指定[用户数据目录](https://chromium.googlesource.com/chromium/src/+/master/docs/user_data_dir.md)
  - `env` <[Object]> 指定浏览器可见的环境变量，默认 `process.env`
  - `devtools` <[boolean]> 是否为每个 tab 自动打开 DevTools 面板，如果此参数被设置为 `true`，`headless` 参数强制为 `false`
  - `pipe` <[boolean]> 使用管道代替 WebSocket 连接浏览器，默认 `false`
- 返回值: <[Promise]<[Browser]>> Promise: resolve 时返回浏览器实例

<!-- The method launches a browser instance with given arguments. The browser will be closed when the parent node.js process is closed. -->
该方法根据给定的参数启动一个浏览器实例。当父 node.js 进程关闭时，浏览器将被关闭。


<!-- > **NOTE** Puppeteer can also be used to control the Chrome browser, but it works best with the version of Chromium it is bundled with. There is no guarantee it will work with any other version. Use `executablePath` option with extreme caution. -->
> **NOTE** Puppeteer也可以用来控制 Chrome 浏览器，但它与捆绑在一起的Chromium 版本效果最好。不能保证它可以与任何其他版本一起使用，请谨慎地使用`executablePath`选项。
>
<!-- > If Google Chrome (rather than Chromium) is preferred, a [Chrome Canary](https://www.google.com/chrome/browser/canary.html) or [Dev Channel](https://www.chromium.org/getting-involved/dev-channel) build is suggested. -->
> 如果比较喜欢 Google Chrome (相对 Chromium)， 建议使用 [Chrome Canary](https://www.google.com/chrome/browser/canary.html)  或者 [Dev Channel](https://www.chromium.org/getting-involved/dev-channel)  版本
>
<!-- > In [puppeteer.launch([options])](#puppeteerlaunchoptions) above, any mention of Chromium also applies to Chrome. -->
> 上面的 [puppeteer.launch([options])](#puppeteerlaunchoptions) 中，任何提及 Chromium 的同样适用于 Chrome
>
<!-- > See [`this article`](https://www.howtogeek.com/202825/what%E2%80%99s-the-difference-between-chromium-and-chrome/) for a description of the differences between Chromium and Chrome. [`This article`](https://chromium.googlesource.com/chromium/src/+/lkcr/docs/chromium_browser_vs_google_chrome.md) describes some differences for Linux users. -->
> [本文](https://www.howtogeek.com/202825/what%E2%80%99s-the-difference-between-chromium-and-chrome/)讲述了 Chromium 与 Chrome 之间的差异；[本文](https://chromium.googlesource.com/chromium/src/+/lkcr/docs/chromium_browser_vs_google_chrome.md) 讲述了 Linux 上的一些差异。
